...
CSV_OUTPUT_PATH = "../finances/financial_report.csv"
...
def get_email_data(service, num_emails):
    ...
    while len(emails) < (num_emails):
        ...
        results = service.users().messages().list(userId='me', q='Bank of America',
...
def export_to_postgresql(emails):
    ...
    for email in emails:
        insert_query = """
        INSERT INTO transaction_emails (received_at, amount, category)
        VALUES (%s, %s, %s)"""
