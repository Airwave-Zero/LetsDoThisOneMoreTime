    ...
    for year_folder in os.listdir(start_dir):
        ...
        for job_board_folder in os.listdir(year_path):
            ...
            if not os.path.isdir(job_board_path):
                continue
            raw_silver_path = os.path.join(job_board_path, "raw_silver")
            combined_silver_path = os.path.join(job_board_path, "combined_silver")
            # TODO: check based on memory instead of just solely checking existence
            if os.listdir(combined_silver_path).count(".parquet") > 0:
                logging.info(f"Combined parquets already exists for {job_board_path}, skipping combination.")
                continue
            else:
                process_and_chunk_parquets(raw_silver_path, job_board_folder, combined_silver_path, target_mb=300)

def process_and_chunk_parquets(folder_path, job_board_folder_name, output_silver_path, target_mb=300):
    '''
    ...
    for file in parquet_files:
        ...
        while start < total_rows:
            ...
            if current_rows >= rows_per_file:
                ...
                output_file = os.path.join(output_silver_path,f"combined_{job_board_folder_name}_part{part_num}.parquet")
    ...
    for each_file in os.listdir(start_dir):
        ...
        if os.path.isfile(file_path) and each_file.endswith(".parquet"):
            if each_file.startswith("cleaned_"):
                ...
                new_folder_path = os.path.join(start_dir, year, job_board, "raw_silver")
                combined_silver_path = os.path.join(start_dir, year, job_board, "combined_silver")
                os.makedirs(new_folder_path, exist_ok=True)
                os.makedirs(combined_silver_path, exist_ok=True)
